...
def merck_r_square(data_pred, data_label):
	label_mean = np.mean(data_label)
	pred_mean = np.mean(data_pred)
	numerator = np.sum((data_pred - pred_mean) * (data_label - label_mean)) ** 2
	denominator = np.sum((data_pred - pred_mean) ** 2) * np.sum((data_label - label_mean) ** 2)
